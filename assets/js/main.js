...
const ParticleCanvas = (() => {
  ...
  class Particle {
    ...
    reset(w, h) {
      ...
      this.color = Math.random() > 0.5 ? '0, 87, 255' : '124, 58, 237';
    }
    update(w, h) {
      this.x += this.speedX;
      this.y += this.speedY;
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < maxDist) {
          ...
          ctx.strokeStyle = `rgba(0, 87, 255, ${opacity})`;
          ctx.lineWidth = 0.8;
          ctx.stroke();
        }
      }
...
const AIChatbot = (() => {
  const responses = {
    ...
    'price': 'Our pricing starts from $999 for basic apps and custom packages for enterprise solutions. Would you like a detailed quote? 💰',
    'pricing': 'Our plans range from Starter ($999) to Professional ($4,999) to Enterprise (custom). Each includes source code, 6-month support, and free deployment!',
    'contact': 'You can reach us at:\n📧 info@riskfreefeel.com\n📞 +1 (555) 123-4567\n💬 WhatsApp: +1 (555) 123-4567\n\nOr fill the contact form on our website!',
